...
export async function selectRatedProfileByProfileId(profileId: string) : Promise<PartialProfile|null> {
  try {
    ...
    const mysqlQuery : string = "SELECT BIN_TO_UUID(profileId) AS profileId, profileEmail, profileName, (SELECT AVG(ratingAmount) FROM rating WHERE profile.profileId = rating.ratingProfileId1) AS ratingAverage, (SELECT COUNT(*) FROM rating WHERE profile.profileId = rating.ratingProfileId1 AND ratingContent IS NOT NULL) AS ratingCount FROM profile INNER JOIN rating ON rating.ratingProfileId1 = profile.profileId WHERE profileId = UUID_TO_BIN(:profileId)"
