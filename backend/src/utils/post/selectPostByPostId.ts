...
export async function selectPostByPostId(postId: string) : Promise<Post|null> {
    try {
        ...
        const mySqlQuery = "SELECT BIN_TO_UUID(postId) AS postId, BIN_TO_UUID (postProfileId) AS postProfileId, postActive, postCategory,postContent, postDate, postPicture from post WHERE postId = UUID_TO_BIN(:postId)"
